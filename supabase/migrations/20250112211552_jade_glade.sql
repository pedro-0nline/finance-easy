...
BEGIN
  ...
  AND s.status IN ('active', 'trialing')  -- Only get active or trialing subscriptions
...
-- Add helpful comment
COMMENT ON FUNCTION get_user_stripe_subscription IS 'Get Stripe subscription details by joining user email with stripe_customer and accessing subscription data';