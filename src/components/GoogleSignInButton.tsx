...
interface GoogleSignInButtonProps {
  /** Optional callback to surface errors to the parent form. */
  onError?: (message: string) => void;
}

/** Official Google "G" logo (multi-color), inline SVG so there are no broken assets. */
function GoogleIcon() {
  return (
...
export function GoogleSignInButton({ onError }: GoogleSignInButtonProps) {
  ...
  const handleClick = async () => {
    ...
    onError?.('');
    try {
      // Redirects away to Google; on failure we stay on the page and report.
  ...
  return (
    ...
    >
      {isLoading ? <Loader2 className="animate-spin h-5 w-5" /> : <GoogleIcon />}
